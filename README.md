...
m = next(x for x in pv["macros"] if x["name"] == "confabsorbmean")
# m: {"value": "60.3", "scope": "absorption_cohort_14",
#     "key_paths": ["control_sensitivity.none", "control_sensitivity.standard"],
#     "formula": "mean over scope of (none - standard) / none * 100",
#     "source_files": [<14 files>]}
vals = []
for f in m["source_files"]:
    d = json.load(open(f"results/{f}"))
    raw, std = d["control_sensitivity"]["none"], d["control_sensitivity"]["standard"]
    vals.append((raw - std) / raw * 100)
print(np.mean(vals))   # 60.3173 -> 60.3 at one decimal
```
...
A Croissant 1.1 metadata descriptor at `croissant.json` covers the full results inventory: 4 FileObjects (parent archive, model and dataset revision manifests, latest verification report), 13 FileSets (one per result file type, glob-matched against `results/`), and 13 RecordSets with fields derived from `schema/*.schema.json`. The descriptor is regenerated by `just croissant` and gated by `just check-croissant`, which runs the official MLCommons validator. Spec: <https://docs.mlcommons.org/croissant/docs/croissant-spec-1.1.html>.
...
scripts/              GPU experiment launchers (run_model.py is the entry point);
                      schema validator and scope/report exporters.
analysis/             CPU statistical analysis (permutation test, mixed-effects, schema validation)
schema/               Formal JSON Schema (Draft 2020-12) for every result type
                      under results/. Dispatch matrix in
                      scripts/validate_schemas.py:DISPATCH. CI-enforced.
results/              All result JSONs (committed, reproducible, schema-validated).
                      Includes manifests (model_revisions, dataset_revisions) and
                      manifest_verification/ audit reports.
reports/              Cross-repo claim provenance:
                        paper_values.json   - paper macros with source-file,
                                              key-path, formula, scope
                                              annotations; coverage locked to
                                              grow, not regress
                        scopes.json         - named cohort definitions
                        figure_sources.json - PDF -> source JSON mapping
                      Generated, drift-checked in CI.
tests/                Schema, metrics, analysis smoke, probe-sync drift guards,
                      paper_values integrity, direct-read auto-verification,
                      figure-source existence, formal schema validation.
