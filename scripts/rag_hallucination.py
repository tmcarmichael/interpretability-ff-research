...
if not torch.cuda.is_available():
    sys.exit(
        "rag_hallucination.py produces paper-quality results and requires CUDA. "
...
def load_nq(max_questions=1000):
    ...
    ds = load_dataset(
        "rajpurkar/squad_v2", split="validation", revision=DATASET_REVISIONS["rajpurkar/squad_v2"]["commit"]
    )
    questions = []
    for row in ds:
...
OUT_PATH = _resolve_out(args.output or f"{HF_SLUG}_squad-rag.json")
